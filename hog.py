...
def boar_brawl(player_score, opponent_score):
    ...
    points = 3 * abs(tens_digit - ones_digit)
    # END PROBLEM 2
    return max(points, 1)
...
def sus_points(score):
    ...
    assert num_factors(score) == 2, "Incorrect value for prime number"
    # END PROBLEM 4
    return score
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    for i in range(num_rolls):
        points += dice()
        if num_factors(points) in [3, 4]:
            points = sus_points(points)
    # END PROBLEM 4
    return points
...
def play(strategy0, strategy1, update,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    winner = lambda x : x >= goal

    while not(winner(score0) or winner(score1)):
        if who == 0:
